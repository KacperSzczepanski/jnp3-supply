...
streamToList (a :> b) = a : streamToList b


instance Show a => Show (Stream  a) where
  show stream = showStream (take 20 $ streamToList stream) where
    showStream [] = "..."
    showStream (a:as) = show a ++ "," ++ showStream as
...
pureSupply a = S {runSupply = (\s -> (a, s))}
